...
| `app.js` | Page switching, rendering, filtering, saving, RSVP behavior, profiles, map behavior, the generic `liveOpportunity*` functions that read the registry and render a live-feed section (including the generic `postingKind: "official_program"` card) on a matching employer's detail page, and (`LIVE`, added 2026-07-14) lightweight History API routing — see [Browser Navigation](../features/browser-navigation.md) |
...
| Browser Back/Forward navigation | Lightweight `history.pushState`/`replaceState`/`popstate` routing added 2026-07-14, using `?view=<page>[&employer=<id>]` query-string routes on the same static document (no framework, no path-based routes, no new dependency). Covers Home, Find Opportunities, Employer detail, Employer Map, Third Spaces, and Community Hub. See `docs/features/browser-navigation.md` | `LIVE` |
