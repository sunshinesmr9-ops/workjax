...
| `applicationTiming` | string enum | `"annual_recurring"` | Not yet rendered; exists as a structured-date foundation. Values on the 38 current records: `annual_recurring` (1), `fixed_dated` (2), `seasonal_window` (9), `rolling` (21), `unknown` (5), per `docs/data/date-normalization-audit.md` | Opportunity (no direct target field; closest is a derived value of `application_open_at`/`application_close_at`) | Keep | Added `LIVE` in this task; classification copied verbatim from the audit, not invented | No |
| `applicationOpenAt` | value (currently always `null`) | `null` on all 38 records | `isOpportunityActive()` in `app.js` (only meaningful once non-null and verified) | Opportunity.`application_open_at` | Keep | `null` on every current record — no year or date exists in the source text | No |
| `applicationCloseAt` | value (currently always `null`) | `null` on all 38 records | `isOpportunityActive()` in `app.js` | Opportunity.`application_close_at` | Keep | Same as `applicationOpenAt` | No |
| `dateVerificationStatus` | string | `"unverified"` on all 38 records | `isOpportunityActive()` in `app.js` — a record is treated as active whenever this is not `"verified"` | Opportunity (closest: `last_verified_at`/`status`) | Keep | `"unverified"` on every current record; no record has been through a verification process | No |
...
| `experienceType` | string enum or `null` | `"recurring_space"` | Not yet rendered; exists as a structured-date foundation. Values on the 15 current records: `recurring_space` (8), `scheduled_event` (1), `null` (6) — `null` marks records the audit flags as needing a future split (e.g. Cody Johnson Live '26) or as evergreen/standing activities that don't fit the current two-value enum (e.g. Kayaking) | Experience.`experience_type` | Keep | Added `LIVE` in this task; value copied verbatim from the audit's recommendation column, not invented | No |
| `startsAt` | value (currently always `null`) | `null` on all 15 records | `isEventActive()` in `app.js` (only meaningful once non-null and verified) | Experience.`starts_at` | Keep | `null` on every current record, including Cody Johnson Live '26 and The Music of David Bowie (Symphonic) | No |
| `endsAt` | value (currently always `null`) | `null` on all 15 records | `isEventActive()` in `app.js` | Experience.`ends_at` | Keep | Same as `startsAt` | No |
| `recurrenceRule` | value (currently always `null`) | `null` on all 15 records | Not yet rendered | Experience.`recurrence_rule` | Keep | `null` on every current record; no machine-readable rule has been authored yet | No |
| `dateVerificationStatus` | string | `"unverified"` on all 15 records | `isEventActive()` in `app.js` — a record is treated as active whenever this is not `"verified"` | Experience (closest: `last_verified_at`/`status`) | Keep | `"unverified"` on every current record | No |
...
| 2026-07-13 | Documented new structured-date foundation fields: `applicationTiming`/`applicationOpenAt`/`applicationCloseAt`/`dateVerificationStatus` on `employers`; `experienceType`/`startsAt`/`endsAt`/`recurrenceRule`/`dateVerificationStatus` on `events`. All current values are `null`/`"unverified"`; no record is auto-removed. | Claude (documentation task) |
