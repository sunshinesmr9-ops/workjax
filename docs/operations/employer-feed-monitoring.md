...
- **iCIMS public-portal qualification (generic, driven by the registry entry's own `employerName`):** a record must explicitly identify the configured employer (e.g. "Miller Electric Company" matches "Miller Electric" wherever it appears — never a generic EMCOR mention or an unrelated EMCOR subsidiary), and must satisfy both of the following — not just one:
  - **Location evidence:** the visible listing explicitly includes Jacksonville, "Jacksonville, FL", or "US-FL-Jacksonville".
  - **Strong student evidence:** position type is Intern, category is Internship, the title explicitly contains intern/internship/co-op/apprentice/apprenticeship, or the listing explicitly states it is for a currently enrolled college/university student or early-career program participant. An ordinary full-time or "entry-level" listing never qualifies on its own, and high-school eligibility is never inferred. Ordinary Jacksonville full-time listings for the configured employer are still counted in "employer records found" and "Jacksonville employer records found" — they just don't qualify without a strong student signal.
  - See "iCIMS public-portal parse outcomes" below for how a genuinely empty result is distinguished from a parsing failure — the two are never conflated.
...
## iCIMS public-portal parse outcomes

A successful HTTP response from an iCIMS public-portal page is **not** by itself evidence of a healthy parse — early monitoring of Miller Electric's source showed a page that fetched successfully and visibly contained job records, while the parser extracted zero, and was incorrectly reported as a healthy, empty result. To prevent that recurring, every `icims_public_portal` check now resolves to one of four explicit outcomes, carried in a `parseStatus` field on the source's result (in addition to the existing `healthy` boolean, kept for compatibility):

| `parseStatus` | `healthy` | Meaning |
|---|---|---|
| `healthy_with_records` | `true` | At least one recognizable job record was parsed from the page(s) checked. |
| `healthy_empty` | `true` | Zero records were parsed, **and** the page explicitly and reliably indicates zero results (e.g. a visible "no jobs found" or "your search did not match any jobs" message). |
| `parser_warning` | `false` | Zero records were parsed, but there was **no** explicit zero-results message — either the page shows recognizable job-results structure (iCIMS_Job\*-prefixed classes, `data-job-id`/`data-jobid` attributes, or an "N of M jobs" count) that the parser still failed to extract, or the page's structure couldn't be recognized as a results page at all. Either way, this is a warning for manual review, not a valid zero-opportunity result — it appears under "Sources that failed" in the report, `healthy: false`. |
| `request_failure` | `false` | The page could not be usefully fetched at all: a network error, timeout, a hostname that leaves the allowed iCIMS domain after redirects, a non-2xx HTTP status, or a detected bot-challenge/verification page. |

**Absence of parsed records is never, by itself, evidence of zero jobs.** `healthy_empty` requires an explicit, matched "no results" phrase in the page; everything else with zero records is `parser_warning`. This is the core fix — a real parsing problem can no longer silently masquerade as "healthy, nothing found."

**Response-size safety limit:** the fetch reads the response through a capped, streamed reader — currently 5,000,000 bytes (`ICIMS_MAX_HTML_BYTES`, raised from an earlier 2,000,000-byte cap as headroom, not because truncation was confirmed as the cause of any specific empty result). This is still a hard, finite limit, not an unbounded read; every result records whether the cap was actually reached (`diagnostics.responseSizeCapReached`), so a future truncation-caused failure is directly observable rather than guessed at.

**Safe diagnostics** — every iCIMS result carries a `diagnostics` object, shown in both the workflow logs and the "Employer Feed Watch Report" issue (this script always renders the same report to both): `responseBytesRead`, `responseSizeCapReached`, `finalUrl`/`finalHostname`, `resultsPageMarkerFound`, `zeroResultsMarkerFound`, and `potentialJobLinksFound` (raw job-detail-link matches before employer/location/student filtering), alongside the existing `pagesChecked`, `recordsExamined`, `employerRecordsFound`, `jacksonvilleEmployerRecordsFound`, and `qualifying` count. None of this ever includes cookies, tokens, or full page HTML.

**A `parser_warning` must be repaired before another iCIMS employer is added.** Because `icims_public_portal` is a shared, generic adapter, a known, unresolved extraction failure on one employer's page is a signal the adapter itself needs fixing — not something to work around by adding a second employer on top of it. Treat a `parser_warning` on any iCIMS source as a blocker for onboarding another one (e.g. Haskell) until it's resolved or the specific portal-structure difference is understood and handled.
...
| 2026-07-14 | Repaired and hardened the `icims_public_portal` adapter after a report showed a confirmed-visible results page parsed as zero records yet "healthy": broadened the job-detail-link pattern to also match relative hrefs, added a four-outcome `parseStatus` model (`healthy_with_records`/`healthy_empty`/`parser_warning`/`request_failure`) so an unparsed-but-visible results page is never reported as a clean empty result, added safe per-source diagnostics, and genericized employer-identity matching from the registry's own `employerName` (no employer-specific logic remains hardcoded) | Claude (implementation task) |
